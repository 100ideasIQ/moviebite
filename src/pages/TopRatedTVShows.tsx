...
const TopRatedTVShows: React.FC = () => {
  ...
  const fetchTopRatedTVShows = async (page: number) => {
    setLoading(true);
    try {
      ...
      if (page === 1) {
        ...
        setShows(prev => [...prev, ...data.results.map((show: any) => ({ ...show, media_type: 'tv' }))]);
      }
  ...
  const loadMore = () => {
    if (currentPage < totalPages && !loading) {
      fetchTopRatedTVShows(currentPage + 1);
    }
  };
    ...
    <div className="min-h-screen container mx-auto px-4 lg:px-8 py-4 lg:py-8">
      ...
      >
        <div className="flex items-center space-x-3 mb-4 lg:mb-6">
          ...
          <h1 className="text-2xl lg:text-4xl font-bold text-white">Top Rated TV Shows</h1>
        </div>
        <p className="text-gray-400 text-sm lg:text-base">
          Discover the highest-rated TV shows of all time
        </p>
      </motion.div>
      ...
      {loading && shows.length === 0 ? (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm lg:text-base">Loading top rated TV shows...</p>
          </div>
        </div>
      ) : (
        <>
